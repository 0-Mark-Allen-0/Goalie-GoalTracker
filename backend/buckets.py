...
async def bucket_helper(bucket) -> dict:
    ...
    # Fetch all Goals associated with this Bucket
    goals_cursor = goals_collection.find({"bucket_id": bucket_id, "user_id": bucket["user_id"]})
...
async def get_buckets(user=Depends(get_current_user)):
    buckets_cursor = buckets_collection.find({"user_id": user["sub"]})
...
async def update_bucket(id: str, data: dict, user=Depends(get_current_user)):
    ...
    if not existing_bucket:
        raise HTTPException(status_code=404, detail="Bucket not found")
    if existing_bucket["user_id"] != user["sub"]:
...
async def delete_bucket(id: str, user=Depends(get_current_user)):
    ...
    # Data Integrity Check - Prevent delete if there are Goals still attached to it
    attached_goals_count = await goals_collection.count_documents({"bucket_id": id})
